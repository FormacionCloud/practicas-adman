...
Para crear la release, será necesario dar *permisos de escritura en el repositorio* a la Action de GitHub. Para ello, habrá que utilizar el *token* de GitHub y cambiar sus *permisos*. Para ello, accede a los *ajustes del repositorio* y modifica los permisos en la parte final de la pantalla:
[[./imagenes/actions-permisos-1.png]]

[[./imagenes/actions-permisos-2.png]]

A continuación se facilita un ejemplo de cómo debería quedar el fichero de GitHub Actions:
#+begin_src yaml
  name: App Flutter
  ...
  on:
    push:
      branches:
        - master
  env:
    GH_TOKEN: ${{ github.token }} # token de GitHub para tener permisos para crear la Action
  jobs:
    flutter:
      ...
      steps:
        ...
        - name: Ejecutar tests
          run: flutter test
        - name: Construir APK y AAB
          run: flutter build apk --release
        - run: flutter build appbundle --release
        - name: Crear release
          run: gh release create "v1.0.0"
        - name: Subir archivos a release
          run: gh release upload "v1.0.0" build/app/outputs/flutter-apk/app-release.apk build/app/outputs/bundle/release/app-release.aab
#+end_src
