...
## 3. Recursos IAM: Rol para la instancia
...
En nuestro caso, no hay una necesidad inmediata, pero con fines didácticos ilustramos cómo debería definirse para tener permisos de comunicación con Systems Manager (SSM) para facilitar administración, utilizando un **Rol IAM**. Recuerda que este apartado no podría realizarse en los entornos de laboratorio de Academy. No obstante, como vamos a utilizar la **Landing Zone**, sí que podremos hacerlo.

### Definición del rol IAM
A continuación, analizaremos cómo crear un perfil de instancia para asociar a las instancias de la aplicación. El objetivo es que las instancias tengan permisos para trabajar con Systems Manager Session Manager y poder iniciar sesión remota a través de dicho servicio. Este código está definido en el archivo **`rol.tf`**.
...
- **aws_iam_role.web_role:** Crea un rol IAM nombrado con el prefijo de proyecto. `assume_role_policy` es el trust policy que permite al servicio EC2 asumir este rol (principal `ec2.amazonaws.com`). Esto es imprescindible para que EC2 pueda usar el rol.
- **aws_iam_role_policy_attachment.web_policy_role_attach:** Adjunta la política AWS predefinida **AmazonSSMManagedInstanceCore** al rol, para que la instancia tenga permisos de SSM (por ejemplo, para usar Session Manager en lugar de SSH, enviar logs, etc.).
- **aws_iam_instance_profile.web_profile:** Crea un instance profile, que es la entidad asociable a EC2, alimentada por el rol.
...
# Si es la primera máquina, instala y configura wordpress
