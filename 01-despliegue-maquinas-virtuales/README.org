...
- Crea un *componente personalizado* basado en el siguiente documento, realizando los cambios que se indican a continuación:
    ...
    phases:
      - name: build
        steps:
          ...
          # Deja el código comentado, dado que Amazon Linux 2023 no es compatible con esta acción
      - name: test
        steps:
  ...
  *Añade un paso* para *actualizar el sistema* operativo mediante la [[https://docs.aws.amazon.com/imagebuilder/latest/userguide/toe-action-modules.html#action-modules-updateos][acción UpdateOS]]. Deja el código *comentado*, dado que Amazon Linux 2023 no es compatible con esta acción.
