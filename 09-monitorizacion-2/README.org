...
En esta práctica realizaremos una segunda parte del Workshop [[https://catalog.workshops.aws/observability/en-US][One Observability Workshop]] de AWS. La sección de "Instalación y configuración previa" es la misma que la de la práctica "Observabilidad I".
...
* Pasos a realizar
Se supone que estás familiarizado con la arquitectura. En caso contrario, [[https://catalog.workshops.aws/observability/en-US/architecture][explora la arquitectura y la aplicación web]] en la sección correspondiente.

A continuación, realiza las siguientes *tareas* del taller:
- Sección *AWS native Observability / Application performance monitoring / X-Ray traces*, completa, con todas sus subsecciones
- Sección *AWS native Observability / Application performance monitoring / CloudWatch Synthetics*, hasta la subsección *Canary Alarms*, incluida
- Sección *AWS native Observability / Application performance monitoring / CloudWatch RUM*, completa, con todas sus subsecciones
- Sección *AWS native Observability / Insights / Lambda Insights*, completa, con todas sus subsecciones
- Sección *AWS native Observability / Insights / Application Insights*, completa, con todas sus subsecciones
- Sección *AWS native Observability / Dashboards*, hasta la subsección *CloudWatch Automatic Dashboards*, incluida
- Sección *AWS managed open-source Observability / AWS Distro for OpenTelemetry / AWS Distro components* 
- Sección *AWS managed open-source Observability / AWS Distro for OpenTelemetry / Tracing Lambda functions*
...
* Limpieza
Recuerda *eliminar toda la infraestructura* al terminar, siguiendo las [[https://catalog.workshops.aws/observability/en-US/cleanup#delete-all-resources][instrucciones oficiales]]. Es necesario ejecutar el siguiente comando en CloudShell. Igual que ocurre con la instalación, la eliminación de recursos también llevará un tiempo considerable. Recuerda confirmar en *CloudFormation* que se han eliminado todos los stacks. En caso contrario, *ejecuta de nuevo el script de desinstalación*.

#+begin_src bash
curl https://raw.githubusercontent.com/pedroprieto/one-observability-demo/main/PetAdoptions/cdk/pet_stack/resources/destroy_stack.sh | bash
#+end_src

*Adicionalmente*, *borra manualmente* los siguientes recursos que habrás creado durante la práctica:
- Cognito Identity Role
- Cloudwatch RUM app monitor
- Cloudwatch Synthetics Canary (previamente deberás detenerlo y a continuación borrarlo)
- Cloudwatch alarms
