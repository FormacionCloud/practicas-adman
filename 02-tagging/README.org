...
- [[https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_policies_examples_ec2-start-stop-match-tags.html][EC2: Start or stop instances based on matching principal and resource tags]]
...
- Crea una *política IAM asociada a identidad* que permita *iniciar o detener instancias EC2* siempre que la *etiqueta de la identidad y la de la máquina* tengan el *mismo valor* para la etiqueta ~TUS_INICIALES:service~. Añade también una *cláusula* que incluya los permisos ~DescribeInstances~ para todas las instancias con el fin de poder visualizarlas al cambiar de rol
...
** Limpieza
Al finalizar, *elimina los recursos creados* en la práctica. Asegúrate de borrar las *instancias* y la *regla de AWS Config*.
